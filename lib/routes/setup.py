...
def status(c: Conn = Depends(get_conn)):
    ...
    from .. import fixes
    return {"needed": _needed(c), "migrated": _migrated(c), "applied": fixes.aplicadas(c)}


def _migrated(c: Conn) -> bool:
    if not c.pg:
        return True  # SQLite rehace las tablas desde schema.sql en cada conexión
    from ..db import _ESPERADAS, _SENTINEL, _SENTINEL_ARGS
    return c.query(_SENTINEL.replace("%s", "?"), _SENTINEL_ARGS)[0]["count"] == len(_ESPERADAS)
